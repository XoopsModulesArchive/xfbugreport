...
include('../../header.php');
$GLOBALS['xoopsOption']['template_main'] = 'xfmod_bugadd.html';
...
if ('postadd' == $func) {
    ...
    if (empty($user_email)) {
        ...
        if (!checkEmail($user_email)) {
            redirect_header($GLOBALS['HTTP_REFERER'], 2, _XF_BR_INVALIDMAIL);
    ...
    */
	...
	<TABLE>
	<TR><TD VALIGN="TOP"><B>' . _XF_BR_CATEGORY . ':</B><BR>';

        $content .= $ath->categoryBox('category_id');
        $content .= '&nbsp;<A HREF="' . XOOPS_URL . '/modules/xfmod/tracker/admin/?group_id=' . $group_id . '&atid=' . $ath->getID() . '&add_cat=1">(' . _XF_BR_ADMINSMALL . ')</A>';
        $content .= '</TD><TD><B>' . _XF_BR_GROUP . ':</B><BR>';
        $content .= $ath->artifactGroupBox('artifact_group_id');
        $content .= '&nbsp;<A HREF="' . XOOPS_URL . '/modules/xfmod/tracker/admin/?group_id=' . $group_id . '&atid=' . $ath->getID() . '&add_group=1">(' . _XF_BR_ADMINSMALL . ')</A>';
    ...
    if ($ath->userIsAdmin()) {
        ...
        $content .= '&nbsp;<A HREF="' . XOOPS_URL . '/modules/xfmod/tracker/admin/?group_id=' . $group_id . '&atid=' . $ath->getID() . '&update_users=1">(' . _XF_BR_ADMINSMALL . ')</A>';
    ...
    $content .= '<TR><TD COLSPAN="2"><B>' . _XF_BR_SUMMARY . ':</B><BR>
		<INPUT TYPE="TEXT" NAME="summary" SIZE="50" MAXLENGTH="70">
	</TD></TR>

	<TR><TD COLSPAN="2">
		<B>' . _XF_BR_DETAILEDDESCRIPTION . ':</B>
		<P>
    ...
    if (!$xoopsUser) {
        ...
        $content .= "<h4><FONT COLOR='RED'>" . _XF_BR_PLEASE
             . " <a href='" . XOOPS_URL . '/user.php?xoops_redirect=' . $_SERVER['PHP_SELF'] . '?' . urlencode($_SERVER['QUERY_STRING']) . "'>" . _XF_BR_LOGIN . '</A></FONT></h4><BR>'
             . _XF_BR_IFCANNOTLOGIN . ':<P>'
             . "<INPUT TYPE='TEXT' NAME='user_email' SIZE='30' MAXLENGTH='35'>";
    }

        $content .= '<P>
		<H4><FONT COLOR=RED>' . _XF_BR_DONOTENTERPASSWORDS . '</FONT></H4>
		<P>
	</TD></TR>

	<TR><TD COLSPAN=2>
		<B>' . _XF_BR_CHECKTOUPLOAD . ':</B> <input type="checkbox" name="add_file" VALUE="1">
		<P>
		<input type="file" name="input_file" size="30">
		<P>
		<B>' . _XF_BR_FILEDESCRIPTION . ':</B><BR>
